...
`# Добро пожаловать в hinomaOS! 👋

Это твоя персональная операционная система прямо в браузере.

## Что тут есть

- **Файлы** — проводник в стиле macOS с функциями Windows Explorer
- **Терминал** — настоящая командная строка (набери \`help\`)
- **Paint, Сапёр, Калькулятор** — классика Windows
- **Заметки, Фото, Браузер** — повседневные приложения
...
| Ctrl + Alt + ↑ | Развернуть окно |
| Ctrl + Alt + ↓ | Свернуть окно |
| Alt + Tab | Переключение окон |
| F2 | Переименовать файл |

Перетащи окно к краю экрана — сработает снэп, как в Windows 11.
Наведи курсор на зелёную кнопку окна — появятся раскладки Snap Layouts.
...
Веб-десктоп: эстетика macOS + функциональность Windows.

Собрано на чистом JavaScript — без фреймворков и сборщиков.
